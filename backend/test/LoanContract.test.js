...
let provider,
  ...
  client1,
  client2,
  deployedLoanContractInstance,
  deployedContractNFTInstance,
  mintedTokenId,
  mintedTokenId2,
  requestedLoanId,
  requestedLoanId2;
...
describe(loanContractName || " Contract test", () => {
  before(async () => {
    ...
    const nftContractPath =
      "contracts/" + nftContractName + ".sol:" + nftContractName;
    const nftContractFactory = await ethers.getContractFactory(
      nftContractPath,
      contractOwner
    );
    deployedContractNFTInstance = await nftContractFactory.deploy(
      "ORT NFT",
      "ORT",
      100
    );
    ...
    const loanContractPath =
      "contracts/" + loanContractName + ".sol:" + loanContractName;
    const loanContractFactory = await ethers.getContractFactory(
      loanContractPath,
      contractOwner
    );
    deployedLoanContractInstance = await loanContractFactory.deploy(
      deployedContractNFTInstance.address,
      { value: 1000 }
    );
    ...
    // Mint tokens
    await deployedContractNFTInstance
      .connect(client1)
      .safeMint("Name", "Desc", "image_uri", { value: mintPrice });
    mintedTokenId = (await deployedContractNFTInstance.identifier()) - 1;
    await deployedContractNFTInstance
      .connect(client2)
      .safeMint("Name 2", "Desc 2", "image_uri_2", { value: mintPrice });
    mintedTokenId2 = (await deployedContractNFTInstance.identifier()) - 1;

    console.log(
      `Client1 ${
        client1.address
      } minted token ${mintedTokenId} and has balance ${await provider.getBalance(
        client1.address
      )}`
    );
  });
  ...
  it("Check getLoanStatus - Request loan and get status - returns PENDING", async () => {
    await deployedLoanContractInstance
      .connect(client1)
      .requestLoan(mintedTokenId);
    requestedLoanId = await deployedLoanContractInstance.loanCounter();
    const status = await deployedLoanContractInstance
      .connect(client1)
      .getLoanStatus();
  ...
  it("Check cannot withdrawLoanAmount if dueDate is not set", async () => {
    await deployedLoanContractInstance
      ...
      .setLoanAmount(requestedLoanId, loanAmount);
    let failed = false;
    try {
      await deployedLoanContractInstance.connect(client1).withdrawLoanAmount();
    } catch (error) {
  ...
  it("Check cannot withdrawLoanAmount if token was not transferred", async () => {
    ...
    await deployedLoanContractInstance
      .connect(contractOwner)
      .setDeadline(requestedLoanId, maxTimeInSeconds);
    let failed = false;
    try {
      await deployedLoanContractInstance.connect(client1).withdrawLoanAmount();
    } catch (error) {
  ...
  it("Check withdrawLoanAmount sends the money properly", async () => {
    await deployedContractNFTInstance
      .connect(client1)
      .safeTransfer(deployedLoanContractInstance.address, mintedTokenId);
    const previousBalance = BigInt(await provider.getBalance(client1.address));
    const tx = await deployedLoanContractInstance
      .connect(client1)
      .withdrawLoanAmount();
    const receipt = await tx.wait();
    const transactionGasSpent = BigInt(
      receipt.cumulativeGasUsed * receipt.effectiveGasPrice
    );
    const balanceAfterWithdraw = BigInt(
      await provider.getBalance(client1.address)
    );
    const balanceIfTransactionCostsWereZero =
      balanceAfterWithdraw + transactionGasSpent;

    //console.log(receipt.events);
    expect(balanceIfTransactionCostsWereZero).to.be.equals(
      BigInt(previousBalance) + BigInt(loanAmount)
    );
  });
  ...
  it("Check takeOwnership - Loan not expired yet then fails", async () => {
    ...
    try {
      ...
      if (
        error.message.includes("The loan's due date hasn't been reached yet")
      ) {
        failed = true;
      } else {
        console.log(error.message);
  ...
  it("Check payment - Pay remaining debt then PAID", async () => {
    const debt = BigInt(
      await deployedLoanContractInstance.connect(client1).getDebt()
    );
    const tx = await deployedLoanContractInstance
      .connect(client1)
      .payment({ value: debt });
    ...
    expect(
      eventContainsArgument(paymentEvent, "_remainingDebt", BigInt(0), (arg) =>
        BigInt(arg)
      )
    ).to.be.true;
  });
  ...
  it("Check takeOwnership - Loan expired then NFT transferred to contract owner", async () => {
    ...
    await deployedLoanContractInstance
      .connect(client2)
      .requestLoan(mintedTokenId2);
    requestedLoanId2 = await deployedLoanContractInstance.loanCounter();
    await deployedLoanContractInstance
      .connect(contractOwner)
      .setDeadline(requestedLoanId2, 100);
    await deployedLoanContractInstance
      .connect(contractOwner)
      .setLoanAmount(requestedLoanId2, loanAmount);
    await deployedContractNFTInstance
      .connect(client2)
      .safeTransfer(deployedLoanContractInstance.address, mintedTokenId2);
